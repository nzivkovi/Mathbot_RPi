...
class Supervisor:
    ...
    def __init__(self, robot_ref, go_to_goal_ctrl, follow_wall_ctrl, avoid_obstacles_ctrl):
        ...
        self.controller_states = {
            self.gtg_ctrl: [(self.at_goal, self.hold_ctrl), (self.at_wall, self.fw_ctrl), (self.at_obstacle, self.ao_ctrl)],
            self.fw_ctrl: [(self.at_goal, self.hold_ctrl), (self.at_obstacle, self.ao_ctrl),
                           (self.free, self.gtg_ctrl)],
            self.ao_ctrl: [(self.at_goal, self.hold_ctrl), (self.free, self.fw_ctrl), (self.at_wall, self.fw_ctrl)],
            self.hold_ctrl: []
        }
...
##        return self.distance_from_goal < (self.robot.specs.base_length / 2)
    ...
    def free(self):
        return self.min_dist > self.robot.sensors[0].specs.range_max

    def at_wall(self):
        return self.min_dist < self.robot.sensors[0].specs.range_max / 1.2
    ...
    def execute(self, dt):
        ...
        self.sensor_distances = self.get_us_distances()
        self.robot.update_ticks()
